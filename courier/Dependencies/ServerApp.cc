...
void ServerApp::initialize(int stage)
{
    ...
    {
        //init
        string buffer = "serverlog.csv";
        logFile.open(buffer, std::ios::out);
        logFile << "Time,Node,Parcel Count\n";  // Header
    }
    if (stage == 0) {
...
void ServerApp::handleMessage(cMessage *msg)
{
    ...
    if (msg->arrivedOn("socketIn")) {
        ...
        logFile << simTime() << msg->getName() << "\n";
    }
    //  if scheduled message
    else if(msg->isSelfMessage())
    {
        //processScheduledPacket(msg);
        //  if schedule flag truck
        if(msg->getName()[0]=='t')
        {
            if(simTime()-lastPkt>10)
            {
                cout << "truck scheduled." << endl;
                cMessage *temp = new cMessage("t");
                simtime_t scheduleTime = simTime()+200;
                scheduleAt(scheduleTime, temp);
                lastPkt = simTime();
            }
        }
    }
...
void ServerApp::processLoraMACPacket(Packet *pk)
{
...
bool ServerApp::isPacketProcessed(const Ptr<const LoRaMacFrame> &pkt)
{
...
void ServerApp::updateKnownNodes(Packet* pkt)
{
    const auto & frame = pkt->peekAtFront<LoRaMacFrame>();
...
void ServerApp::addPktToProcessingTable(Packet* pkt)
{
    const auto & frame = pkt->peekAtFront<LoRaMacFrame>();
...
void ServerApp::processScheduledPacket(cMessage* selfMsg)
{
...
void ServerApp::evaluateADR(Packet* pkt, L3Address pickedGateway, double SNIRinGW, double RSSIinGW)
{
    bool sendADR = false;
...
void ServerApp::receiveSignal(cComponent *source, simsignal_t signalID, intval_t value, cObject *details)
{
    if (simTime() >= getSimulation()->getWarmupPeriod())
    {
